...
class BertForCLS(BertPreTrainedModel):
    def __init__(self, config, params):
        ...
        self.params = params   #对应config.py的config
        self.config = config    #对应BertConfig.from_json_file(config.bert_config_file),实际为bert_congfig.josn文件
        # 预训练模型
        ...
        if params.model_type == 'bilstm':
            ...
            self.lstm = nn.GRU(self.params.embed_dense, lstm_num,
                               ...
                               bidirectional=True)  # 双向
        # 全连接分类组件
        self.cls = nn.Linear(params.embed_dense, params.cls_num)   ###从hidden_size768降维到分类数量
    ...
    def reset_params(self):
        nn.init.xavier_normal_(self.dym_weight)   #初始化动态融合层权重
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None,
        ...
        classifier_logits = self.cls(cls_output)  # [bacth_size*]的概率

        if cls_label is not None:
            class_loss = nn.CrossEntropyLoss()(classifier_logits, cls_label,)    #算交叉熵
